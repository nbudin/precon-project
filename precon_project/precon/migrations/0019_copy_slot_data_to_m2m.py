...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        "Copy Panel slots from ForeignKey to M2M field"
        # Note: Don't use "from appname.models import ModelName". 
        # Use orm.ModelName to refer to models in this application,
        # and orm['appname.ModelName'] for models in other applications.

        panels = orm.Panel.objects.all()
        for panel in panels:
            if panel.slot:
                panel._slot_m2m.add(panel.slot)
                panel.save()

    def backwards(self, orm):
        "Copy *FIRST* Panel slot from M2M field to ForeignKey"

        panels = orm.Panel.objects.all()
        for panel in panels:
            if panel._slot_m2m.count() > 0:
                panel.slot = panel._slot_m2m.first()
                panel.save()
    ...
    models = {
        ...
        u'precon.participant': {
            ...
            'nonce': ('django.db.models.fields.CharField', [], {'default': "'k98xz3'", 'unique': 'True', 'max_length': '6'}),
